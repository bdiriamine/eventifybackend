...
async function seed() {
  ...
  const hallsService    = app.get(HallsService);
  ...
  /* ── Users ── */
  let admin: any, manager: any;

  const findOrCreate = async (email: string, data: any) => {
    const existing = await usersService.findByEmail(email);
    if (existing) { console.log(`   ↳ ${email} already exists`); return existing; }
  ...
  if (existingHalls.length === 0) {
    ...
    for (const h of halls) { await hallsService.create(h as any); }
  ...
  if (existingServices.length === 0) {
    ...
    for (const s of services) { await servicesService.create(s as any); }
  ...
  if (existingProducts.length === 0) {
    ...
    for (const p of products) { await productsService.create(p as any); }
